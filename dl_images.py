...
batch_size2=100
inception_nontrainable_layers_count=205 # количество слоЄв InceptionV3, чьи веса мы не мен€ем при обучении, в процессе переноса обучени€(transfer learning)
nb_epoch=1# количество эпох обучени€ нейронной сети
fc_nb_epoch=10# количество эпох обучени€ классификационной части сети
n_classes=2# количество классов дл€ обучени€
train_path=os.path.join(path,"test")# FIXME train!!!
validation_path=os.path.join(path,'validation')
test_path=os.path.join(path,'test')
path_to_save_np=path# путь дл€ сохранений нумпай-массивов ѕќћ≈Ќя…“≈ Ќј —¬ќ… ѕ”“№,  ”ƒј ’ќ“»“≈ —ќ’–јЌя“№ 10 √Ѕ!!!
########################################################################################################################
########################################################################################################################
...
validation_generator = bottleneck_datagen.flow_from_directory(validation_path,
                                                              ...
                                                              class_mode=None,
                                                              shuffle=False)
bottleneck_features_train = inc_model.predict_generator(train_generator,steps=int(len(train_generator.filenames)/batch_size))# пока не разобралс€ со steps, мб дл€ универсальности опустить этот параметр
...
validation_labels = np.array([0] * int(validation_data.shape[0]/2) + [1] * int(validation_data.shape[0]/2)) # за счЄт отсутсви€ перемешивани€(shuffle=False) в генераторе(flow_from_directory)
fc_model = Sequential()
...
fc_model.save_weights(os.path.join(path_to_save_np,'fc_inception_cats_dogs_250.hdf5')) # сохран€ем веса
...
model.load_weights(weights_filename, by_name=True) # загрузить веса в определЄнные слои по имени (by_name=True)
for layer in inc_model.layers[:inception_nontrainable_layers_count]:
    layer.trainable = False
model.compile(loss='binary_crossentropy',
              optimizer=SGD(lr=1e-4, momentum=0.9),
                #optimizer='rmsprop',
              metrics=['accuracy']) # тонка€ настройка (в первый раз использовали RMSProp, во второй раз используем стохастический градиентный бустинг дл€ того, чтобы веса не слищком сильно обновл€лись)
...
train_datagen = ImageDataGenerator(
        ...
        shear_range=0.2,
        zoom_range=0.2,
        horizontal_flip=True) # здесь происходит аугментаци€ данных, в частности, горизонтальное отражение
...
imgs,labels=pred_generator.next() # загружает изображени€ в генератор и присваивает ей label
